...
class SettingsWindow(
...
) : Window("SessionSwitcher Settings") {
    companion object {
    ...
    private fun makeRequestEditorSection(store: SettingsItem.Store) =
        UISection(
            "Request Editor",
    ...
    private fun makeLoggingLevelSection(store: SettingsItem.Store) =
        UISection(
            "Logging options",
    ...
    private fun makeProjectDataSection(): UISection {
        ...
        val importJsonButton =
            JButton("Import from JSON").also {
                it.addActionListener {
        ...
        val deleteSessionsButton =
            JButton("Delete All Sessions").also {
                it.addActionListener {
                    val data = SessionSwitcher.getApi().persistence().extensionData()
        ...
        val deleteUpdateRulesButton =
            JButton("Delete All Update Rules").also {
                it.addActionListener {
                    val data = SessionSwitcher.getApi().persistence().extensionData()
        ...
        val deleteEverythingButton =
            JButton("Delete Everything").also {
                it.addActionListener {
                    deleteAllData()
                }
            }

        val deleteButtonsPanel =
            JPanel().also {
                it.layout = GridLayout(1, 2, 5, 2)
